...
ull SHANK::solve_dhp(G g, G h, ull N)
{
    ...
    while (i <= n && j <= n)
    {
        if (g_subgroup[i].second==h_coset_g_inverse[j].second) return g_subgroup[i].first+(h_coset_g_inverse[j].first*n);
...
{
    g = _g,p = _p,q = _q,e = _e;
    N=static_cast<ull>(sqrt(static_cast<double>(_q)))+1;
    u_i = G::mod_pow(g, G(q-N), p);
    g_subgroup.clear();
    g_subgroup.reserve(N);
    g_subgroup.emplace_back(0,G(1));
    for (ull i=1; i < N; i++)
    {
        g_subgroup.emplace_back(i,g_subgroup[i-1].second*_g%p);
    }
    std::sort(g_subgroup.begin(), g_subgroup.end(),[](const auto& a, const auto& b)
    {
        if (!(a.second==b.second)) return a.second < b.second;
...
void POHLIG_HELLMAN_SHANK::update(G h)
{
    h_coset_g_inverse.clear();
    h_coset_g_inverse.emplace_back(0, h);
    h_coset_g_inverse.reserve(N);
    for (ull i=1; i < N; i++)
    {
        h_coset_g_inverse.emplace_back(i, h_coset_g_inverse[i-1].second*u_i%p);
    }
    std::sort(h_coset_g_inverse.begin(), h_coset_g_inverse.end(),[](const auto& a, const auto& b)
    {
        if (!(a.second==b.second)) return a.second < b.second;
